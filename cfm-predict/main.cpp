...
private:
    std::string message_;
public:
	FileReadException(const std::string& message) throw() : message_(message) {};
	virtual const char* what() const throw(){
		std::cout << message_ << std::endl;
		return message_.c_str();
	}
...
int main(int argc, char *argv[])
{
	...
	if (argc != 6 && argc != 2 && argc != 5 && argc != 3 && argc != 7 && argc != 8)
	{
		...
		std::cout << std::endl << "output_filename_or_dir (opt):" << std::endl << "The filename of the output spectra file to write to (if not given, prints to stdout), or directory if multiple smiles inputs are given (else current directory)." << std::endl;
	...
	if( !boost::filesystem::exists( config_filename ) ){
		...
		throw FileReadException("Could not find file: " + config_filename);
	}
	...
	if( !boost::filesystem::exists( param_filename ) ){
		...
		throw FileReadException("Could not find file: " + param_filename);
	}
	Param param( param_filename );

	//Check for batch input - if found, read in inchis and set up output directory
	...
	for( ; it != data.end(); ++it ){
		...
		std::ofstream of;
		if( !to_stdout ) {
			if( batch_run ) of.open( (output_dir_str + it->getId() + ".log" ).c_str() );
			else of.open( output_filename.c_str() );
			if( !of.is_open() ) 
...
void parseInputFile(std::vector<MolData> &data, std::string &input_filename ){
	...
	if( !ifs.good() ){
		...
		throw FileReadException("Could not open input file: " + input_filename);
	}
